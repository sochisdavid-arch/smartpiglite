...
export default function DesteteForm() {
  const handleExport = (format: 'pdf' | 'csv' | 'xlsx') => {
        const doc = new jsPDF();
        const tableHead = [['ID Madre', 'Fecha Destete', 'Nº Lechones', 'Peso Total (kg)', 'Destino', 'Observaciones']];
        const tableBody = Array.from({ length: 15 }).map(() => Array(6).fill(''));

        const title = "Formulario de Destete";
        ...
        if (format === 'pdf') {
            ...
            XLSX.writeFile(workbook, `formulario_destete.${format}`);
        }
    };
