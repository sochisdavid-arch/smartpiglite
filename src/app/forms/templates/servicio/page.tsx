...
export default function ServicioForm() {
    const handleExport = (format: 'pdf' | 'csv' | 'xlsx') => {
        const doc = new jsPDF();
        const tableHead = [['ID Madre', 'Fecha', 'Tipo de Servicio', 'ID Macho / Lote Semen', 'Inseminador']];
        const tableBody = Array.from({ length: 15 }).map(() => Array(5).fill(''));

        const title = "Formulario de Servicio";
        ...
        if (format === 'pdf') {
            ...
            XLSX.writeFile(workbook, `formulario_servicio.${format}`);
        }
    };
