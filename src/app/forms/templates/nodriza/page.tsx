...
export default function NodrizaForm() {
    const handleExport = (format: 'pdf' | 'csv' | 'xlsx') => {
        const doc = new jsPDF();
        const tableHead = [['ID Madre Nodriza', 'Fecha Inicio', 'ID Madre Original', 'Nº Lechones', 'Observaciones']];
        const tableBody = Array.from({ length: 15 }).map(() => Array(5).fill(''));

        const title = "Formulario de Nodrizas";
        ...
        if (format === 'pdf') {
            ...
            XLSX.writeFile(workbook, `formulario_nodrizas.${format}`);
        }
    };
